...
def process_files(input_dir, output_dir):
    ...
    for file in filelist:
        ...
        if os.path.exists(output_path):
            print(f"{output_path} already exists, skip processing.")
            continue
        # 加载数据
        ...
        for item in data:
            try:
                ...
                if not has_sar:
                    continue
                if not item['has_sar_llm'] or not item['sar_obj_llm']:
                    item['obj_flag'] = False
                else:
                    obj_label = item['sar_obj']
                    obj_pred = item['sar_obj_llm']
                    print(f"{obj_label},{obj_pred}")
...
def calculate_metrics(output_dir):
    ...
    for file in file_list:
        ...
        for item in data:
            ...
            # 判断是否正确识别对象
            # 条件：模型认为有讽刺（has_sar_llm）且 obj_flag 为 True
            if item.get('has_sar_llm', False) and item.get('obj_flag', False):
                preds.append(1)
            else:
                preds.append(0)
