...
CONFIG = {
    ...
    # --- 模型路径 (请替换为你本地的绝对路径) ---
    "internvl_model_path": "/home/fintune/LLaMA-Factory/OpenGVLab/InternVL2_5-8B-hf", 
...
class QwenEnvironment:
    def __init__(self):
        print(f"正在加载 InternVL3-8B 模型: {CONFIG['internvl_model_path']} ...")
        self.device = CONFIG["device"]
        self.pipeline = pipeline(CONFIG['internvl_model_path'], 
    ...
    def generate(self, query_data, few_shot_examples):
        """构建多模态ICL Prompt并调用InternVL模型生成"""
        # 构建样例提示
        mate_prompt = "样例{}:\n输入：图片配文为:{};图片为: {}\n输出：是否讽刺:{};讽刺对象:{};讽刺解释:{}\n"
        ...
        for i, ex in enumerate(few_shot_examples):
            ...
            example_prompt = mate_prompt.format(
                i+1, ex['text'], IMAGE_TOKEN, has_sar,
                sar_obj,
                sar_exp
            )
        ...
        # 4. 调用InternVL pipeline
        response = self.pipeline((prompt, all_images))
    ...
    def validate_prediction_format(self, prediction):
        """
        校验预测格式是否正确，严格匹配InternVL prompt中的格式。
        期望格式：是否讽刺:值;讽刺对象:值;讽刺解释:值
        如果格式正确，返回 (True, 是否讽刺, 讽刺对象, 讽刺解释)，否则返回 (False, None, None, None)。
        """
        # 严格匹配prompt中的格式
        pattern = r"是否讽刺:([是|否]);讽刺对象:(.*?);讽刺解释:(.*)"
        
        match = re.search(pattern, prediction)
        
        if match:
            sar = match.group(1).strip()
            obj = match.group(2).strip()
            exp = match.group(3).strip()
            return True, sar, obj, exp
        else:
            # 尝试另一种可能的格式（如果模型输出换行）
            pattern_alt = r"是否讽刺:\s*([是|否])\s*;\s*讽刺对象:\s*(.*?)\s*;\s*讽刺解释:\s*(.*)"
            match_alt = re.search(pattern_alt, prediction, re.DOTALL)
            if match_alt:
                sar = match_alt.group(1).strip()
                obj = match_alt.group(2).strip()
                exp = match_alt.group(3).strip()
                return True, sar, obj, exp
...
def main():
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        # 保存模型
        torch.save(policy_net.state_dict(), f"./intern_mlp_bert/policy_net_epoch_{epoch+1}_bertscore.pth")
