...
def parse_satire_response(text):
    ...
    """
    # 正则表达式（兼容多行/多空格排版）
    pattern = r"^是否讽刺：?([^；;]+)[；;]\s*讽刺对象：?([^；;]*?)\s*[；;]\s*讽刺解释：?(.*?)\s*$"

    # 执行匹配
    match = re.match(pattern, text, re.DOTALL)  # re.DOTALL让.匹配换行符（若讽刺解释含换行）

    if match:
        # 提取三组内容
        is_satire = match.group(1).strip()       # 是否讽刺
        satire_target = match.group(2).strip()   # 讽刺对象
        satire_explain = match.group(3).strip()  # 讽刺解释
    else:
        print("无法解析结果")
        print(text)
        
    return {
        "is_satirical": is_satire,
        "satirical_target": satire_target,
        "satirical_explanation": satire_explain
    }
